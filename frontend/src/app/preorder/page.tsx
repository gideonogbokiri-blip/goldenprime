  ...
  return (
    <main className="min-h-screen bg-zinc-950">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                ...
                <div className="relative mb-4">
                  ...
                  <input type="number" min="50" step="50" placeholder="50" value={amount}
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-zinc-800 border border-zinc-700 rounded-lg text-2xl font-mono focus:outline-none focus:border-gold-500 transition-colors" />
                </div>
                {amount && parseFloat(amount) > 0 && coinInfo && (
                  ...
                  >
                    <p className="text-gold-500 font-semibold">You will receive: {gpgAmount} GPG</p>
                    <p className="text-sm text-gray-400">@ ${coinInfo.price} per GoldenPrime Gold Coin</p>
                  </motion.div>
                )}
                <div className="grid grid-cols-4 gap-2 mb-6">
                  {[50, 100, 500, 1000].map((v) => (
                    <motion.button
                      ...
                      className={`py-2 rounded-lg text-sm font-semibold transition-colors ${amount === v.toString() ? 'bg-gold-500 text-black' : 'bg-zinc-800 border border-zinc-700 hover:bg-zinc-700'}`}
                    >
                      ${v.toLocaleString()}
                    </motion.button>
                  ))}
                ...
                <motion.button
                  ...
                  className="w-full bg-gold-500 text-black py-4 rounded-lg font-semibold text-lg hover:bg-gold-400 transition-colors disabled:opacity-50"
                >
                  Continue to Payment
                </motion.button>
              </div>
            ...
            >
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <motion.button
                    ...
                    className={`p-4 rounded-xl border-2 text-left transition-all ${payMethod === 'bank_transfer' ? 'border-gold-500 bg-gold-500/5' : 'border-zinc-700 hover:border-zinc-500'}`}
                  >
                    <div className="font-semibold">Bank Transfer</div>
                    <div className="text-sm text-gray-400">Send from your bank</div>
                  </motion.button>
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    onClick={() => setPayMethod('card')}
                    className={`p-4 rounded-xl border-2 text-left transition-all ${payMethod === 'card' ? 'border-gold-500 bg-gold-500/5' : 'border-zinc-700 hover:border-zinc-500'}`}
                  >
                    <div className="font-semibold">Debit/Credit Card</div>
                    <div className="text-sm text-gray-400">Pay with your card</div>
                  </motion.button>
                </div>
                ...
                <AnimatePresence mode="wait">
                  {payMethod === 'bank_transfer' && (
                    <motion.div
                      ...
                      className="space-y-4"
                    >
                      <p className="text-sm text-gray-400 mb-2">Enter your bank details (saved for future recovery)</p>
                  ...
                  {payMethod === 'card' && (
                    <motion.div
                      ...
                      className="space-y-4"
                    >
                      <p className="text-sm text-gray-400 mb-2">Enter your card details (saved for future recovery)</p>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between"><span className="text-gray-400">Amount</span><span className="font-bold">${parseFloat(amount).toLocaleString()}</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">GPG Coins</span><span className="font-bold text-gold-500">{gpgAmount} GPG</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Price per coin</span><span>${coinInfo?.price || 50}</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Payment</span><span className="capitalize">{payMethod.replace('_', ' ')}</span></div>
                </div>
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setStep('amount')} className="px-6 py-3 border border-zinc-700 rounded-lg font-semibold hover:bg-zinc-800">Back</button>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {step === 'done' && (
            ...
            >
              ...
              <div className="flex gap-3 justify-center mt-6">
                <Link href="/dashboard" className="bg-gold-500 text-black px-8 py-3 rounded-lg font-semibold hover:bg-gold-400">Dashboard</Link>
                <button onClick={() => { setStep('amount'); setAmount(''); }} className="px-8 py-3 border border-zinc-700 rounded-lg font-semibold hover:bg-zinc-800">New Preorder</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {/* Preorder History */}
        {initialLoading ? (
          <div className="mt-10"><SkeletonTable rows={3} /></div>
        ) : preorders.length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
            className="mt-10 bg-zinc-900 border border-zinc-800 rounded-xl p-6"
          >
            <h3 className="text-xl font-semibold mb-4">Your Preorders</h3>
            <div className="space-y-3">
              {preorders.map((p, i) => (
                ...
                >
                  <div>
                    <div className="font-semibold">{p.amount} GPG</div>
                    <div className="text-sm text-gray-400">${parseFloat(p.usd_value || p.amount).toFixed(2)} &middot; {p.metadata?.paymentMethod === 'card' ? 'Card' : 'Bank Transfer'}</div>
                    <div className="text-xs text-gray-500">{new Date(p.created_at).toLocaleString()}</div>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusColors[p.status] || 'text-gray-400'}`}>
